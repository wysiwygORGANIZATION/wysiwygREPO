...
-- PlayList_PlayListMusics [rel11]
alter table `playlistmusics`  add column  `playlist_oid`  integer;
alter table `playlistmusics`   add index fk_playlistmusics_playlist (`playlist_oid`), add constraint fk_playlistmusics_playlist foreign key (`playlist_oid`) references `playlist` (`oid`);


-- PlayListMusics_Music [rel12]
alter table `playlistmusics`  add column  `music_oid`  integer;
alter table `playlistmusics`   add index fk_playlistmusics_music (`music_oid`), add constraint fk_playlistmusics_music foreign key (`music_oid`) references `music` (`oid`);
