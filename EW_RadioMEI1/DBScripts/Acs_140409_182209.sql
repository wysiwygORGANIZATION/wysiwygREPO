...
-- GenremusicMusic [rel2]
create table `music_musicgenre` (
   `genre_2_oid`  integer not null,
   `music_oid`  integer not null,
  primary key (`genre_2_oid`, `music_oid`)
);
alter table `music_musicgenre`   add index fk_music_musicgenre_genre_2 (`genre_2_oid`), add constraint fk_music_musicgenre_genre_2 foreign key (`genre_2_oid`) references `genre_2` (`oid`);
alter table `music_musicgenre`   add index fk_music_musicgenre_music (`music_oid`), add constraint fk_music_musicgenre_music foreign key (`music_oid`) references `music` (`oid`);
create index `idx_music_musicgenre_genre_2` on `music_musicgenre`(`genre_2_oid`);
create index `idx_music_musicgenre_music` on `music_musicgenre`(`music_oid`);
