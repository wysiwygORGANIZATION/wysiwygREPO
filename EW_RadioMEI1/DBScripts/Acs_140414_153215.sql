...
create table `cart_item_music` (
   `cart_item_oid`  integer not null,
   `music_oid`  integer not null,
  primary key (`cart_item_oid`, `music_oid`)
);
alter table `cart_item_music`   add index fk_cart_item_music_cart_item (`cart_item_oid`), add constraint fk_cart_item_music_cart_item foreign key (`cart_item_oid`) references `cart_item` (`oid`);
alter table `cart_item_music`   add index fk_cart_item_music_music (`music_oid`), add constraint fk_cart_item_music_music foreign key (`music_oid`) references `music` (`oid`);
